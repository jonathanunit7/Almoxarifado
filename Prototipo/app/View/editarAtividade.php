...
<div class="container mt-4 mb-5">
  <div class="card shadow-lg border-0">
    <div class="card-header bg-primary text-white">
      <h3 class="mb-0">Atividade - <?=$resultado[0]['id'] ?></h3>
    </div>
    <div class="card-body">
      <form action="../Action/novaAtividade.php" method="POST">
        ...
        <div class="row g-3 mb-4">
          <div class="col-md-4">
            ...
            <input type="text" class="form-control" id="nome" name="nome_atividade" value="<?=$resultado[0]['nome_atividade'] ?>" required>
          </div>

          <div class="col-md-2">
            <label for="data_inicio" class="form-label">Data Inicial</label>
            <input type="date" id="data_inicio" name="data_inicio_emprestimo" class="form-control" value="<?= substr($resultado[0]['data_inicio_emprestimo'], 0, 10) ?>"required>
          </div>

          <div class="col-md-2">
            <label for="data_fim" class="form-label">Data Final</label>
            <input type="date" id="data_fim" name="data_fim_emprestimo" class="form-control" value="<?= substr($resultado[0]['data_fim_emprestimo'], 0, 10) ?>" required>
          </div>

          <div class="col-md-2">
            <label for="hora_inicio" class="form-label">Horário inicial</label>
            <input type="time" id="hora_inicio" name="hora_inicio_emprestimo" class="form-control" value="<?= substr($resultado[0]['data_inicio_emprestimo'], 11, 5) ?>"required>
          </div>

          <div class="col-md-2">
            <label for="hora_fim" class="form-label">Horário Final</label>
            <input type="time" id="hora_fim" name="hora_fim_emprestimo" class="form-control" value="<?= substr($resultado[0]['data_fim_emprestimo'], 11, 5) ?>" required>
          </div>
        </div>
        ...
        <div class="d-grid gap-2">
          <button type="submit" class="btn btn-success btn-lg">Gerar nova atividade e emprestimo</button>
        </div>
